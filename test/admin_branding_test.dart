...
const logoPhoto = 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+jRZkAAAAASUVORK5CYII=';
const backgroundPhoto = 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=';
const artwork = MarketBranding(logo: logoPhoto, background: backgroundPhoto);

Future<void> addAdmin(PosStore store, String market, {String username = 'branch-admin', MarketBranding? branding = artwork}) => store.saveUser(
  name: 'Branch Admin', username: username, password: 'test-admin-pass',
  role: UserRole.admin, active: true, marketId: market, marketBranding: branding,
);

void main() {
  test('Admin branding stays in its market across reopen and backup restore', () async {
    final dir = await Directory.systemTemp.createTemp('bazaar-admin-branding-');
    addTearDown(() => dir.delete(recursive: true));
    final path = '${dir.path}/store.sqlite';
    var store = PosStore(sqlite3.open(path), provisionAdmin: false);
    addTearDown(() => store.dispose());
    await setup(store);
    final branch = store.createMarket(name: 'Branch', currency: 'USD', usdToIqdRate: 131000);
    ...
    store.saveProduct(Product(id: 'milk', marketId: branch, name: 'Milk', barcode: '123',
      category: store.categories.first.id, price: 100, cost: 50, stock: 3));
    final sale = store.checkout({'milk': 1}, paymentMethod: 'cash', tendered: 100);
  ...
  test('Invalid images, unauthorized branding and failed saves change nothing', () async {
    ...
    for (final bad in [
      ...
      await expectLater(addAdmin(store, branch, branding: bad), throwsA(isA<PosException>()));
    }
    await expectLater(store.saveUser(name: 'Cashier', username: 'cashier', password: 'cashier-pass',
      role: UserRole.cashier, active: true, marketBranding: artwork), throwsA(isA<PosException>()));
    // Force failure after the user INSERT to verify the shared transaction rolls back.
    db.execute("CREATE TRIGGER fail_branding BEFORE UPDATE ON markets BEGIN SELECT RAISE(ABORT, 'test failure'); END");
    await expectLater(addAdmin(store, branch), throwsA(isA<SqliteException>()));
    expect(db.select('SELECT id FROM users'), hasLength(1));
    expect(store.markets.firstWhere((m) => m.id == branch).settings.logo, isNull);
    db.execute('DROP TRIGGER fail_branding');
    await addAdmin(store, branch);
    await expectLater(addAdmin(store, branch, branding: const MarketBranding()), throwsA(isA<PosException>()));
    expect(store.markets.firstWhere((m) => m.id == branch).settings.background, backgroundPhoto);
    store.logout();
    await store.login('branch-admin', 'test-admin-pass');
    await expectLater(store.saveUser(name: 'Cashier', username: 'cashier', password: 'cashier-pass',
      role: UserRole.cashier, active: true, marketBranding: artwork), throwsA(isA<PosException>()));
  ...
  test('Clearing images restores defaults and malformed backup artwork is rejected', () async {
    ...
    backup['tables']['markets'][0]['data']['settings']['background'] = 'invalid';
    expect(() => store.validateBackup(utf8.encode(jsonEncode(backup))), throwsA(isA<PosException>()));
    final user = store.users.firstWhere((u) => u.username == 'branch-admin');
    await store.saveUser(id: user.id, name: user.name, username: user.username,
      role: user.role, password: '', active: true, marketBranding: const MarketBranding());
    expect(store.settings.logo, isNull);
    expect(store.settings.background, isNull);
